-- Plan de Trabajo — campo "Resultado del seguimiento" en pdt_actividades.
--
-- ⚠ Proyecto COMPARTIDO con otras aplicaciones. Todo lleva prefijo `pdt_` y es ADITIVO.
--
-- Distinto de la columna `resultado` (20260728h_pdt_actividades_resultado.sql): esa es el
-- valor Positiva/Negativa/No Aplica heredado del histórico viejo. `resultado_seguimiento` es
-- texto libre, propio del nuevo tipo de actividad "Seguimiento": obligatorio en Administración,
-- pero igual que la evidencia no bloquea el guardado — se puede completar después (ver
-- `valorDe` en src/modules/actividades/validators/requisitos.ts).

alter table pdt_actividades add column if not exists resultado_seguimiento text;

-- "Subir Actividad" (pantalla nueva) puede agregar un sector que no estaba programado, para
-- reflejar que se trabajó aunque no viniera de la agenda/Estrategia. `programado` distingue
-- ambos orígenes para poder medir "sectores trabajados vs. programados" más adelante.
alter table pdt_sectores add column if not exists programado boolean;

-- ---------- pdt_espejo_guardar: recibe resultado_seguimiento del árbol que manda la PWA ----------
    ...
    loop
        ...
        loop
            insert into pdt_sectores (
                ...
                guardado_momento, guardado_usuario, programado
            ) values (
            ...
            loop
                ...
                insert into pdt_actividades (
                    ...
                    a#>>'{guardada,usuario}',
                    nullif(a->>'resultado_seguimiento', '')
                );
...
-- ---------- pdt_visitas_en_alcance: devuelve resultado_seguimiento en cada actividad ----------
                ...
                select jsonb_agg(jsonb_build_object(
                    ...
                    'actividades', coalesce((
                        select jsonb_agg(jsonb_build_object(
                            ...
                            'resultado_seguimiento', a.resultado_seguimiento,
                            'materiales', coalesce((
                                select jsonb_agg(jsonb_build_object(
