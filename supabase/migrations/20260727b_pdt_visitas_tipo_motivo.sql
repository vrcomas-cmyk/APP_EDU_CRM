-- Plan de Trabajo — visitas administrativas y de evento (no solo a cliente).
--
-- ⚠ Proyecto COMPARTIDO con otras aplicaciones. Todo lleva prefijo `pdt_` y es ADITIVO.
--
-- `tipo` distingue una visita a cliente de tiempo administrativo o un evento; `motivo` es lo
-- que dice qué es cuando `tipo` no es 'cliente' (cliente/hospital no aplican ahí — no hay a
-- quién visitar). Ausente equivale a 'cliente': el espejo de visitas viejas no cambia de
-- significado.

alter table pdt_visitas add column if not exists tipo   text not null default 'cliente';
alter table pdt_visitas add column if not exists motivo text;

-- ---------- pdt_espejo_guardar: recibe tipo/motivo del árbol que manda la PWA ----------
...
-- ---------- pdt_visitas_en_alcance: devuelve tipo/motivo para pintar el calendario del equipo ----------
