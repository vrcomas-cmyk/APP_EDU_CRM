-- Plan de Trabajo — visitas a un prospecto (todavía no dado de alta en el catálogo).
--
-- ⚠ Proyecto COMPARTIDO con otras aplicaciones. Todo lleva prefijo `pdt_` y es ADITIVO.
--
-- `es_prospecto` marca que `cliente` es texto libre, no un nombre del catálogo: alguien a
-- quien ya se visita y se le pueden generar actividades, pero que aún no está dado de alta.
-- Por eso no tiene zona/ejecutivo/estrategia — esos datos vienen del catálogo, que un
-- prospecto todavía no tiene fila ahí.

alter table pdt_visitas add column if not exists es_prospecto boolean not null default false;

-- ---------- pdt_espejo_guardar: recibe es_prospecto del árbol que manda la PWA ----------
...
-- ---------- pdt_visitas_en_alcance: devuelve es_prospecto para pintar el calendario ----------
