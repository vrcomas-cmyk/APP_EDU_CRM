-- Plan de Trabajo — "Subir Actividad" aplicada a varios sectores: un solo registro.
--
-- ⚠ Proyecto COMPARTIDO con otras aplicaciones. Todo lleva prefijo `pdt_` y es ADITIVO.
--
-- Antes, aplicar una actividad a varios sectores creaba una COPIA por sector (con su propia
-- evidencia por subir cada vez). Ahora es un solo registro que vive en un sector "ancla"
-- (`id_sector`, como siempre) y `sectores_ids` lista TODOS los sectores que cubre — incluido el
-- ancla. Vacío/null = actividad normal de un solo sector, sin cambio de comportamiento.

alter table pdt_actividades add column if not exists sectores_ids text[];

-- ---------- pdt_espejo_guardar: recibe sectores_ids del árbol que manda la PWA ----------
        ...
        loop
            ...
            loop
                ...
                insert into pdt_actividades (
                    ...
                    resultado_seguimiento, sectores_ids
                ) values (
                    ...
                    case when jsonb_typeof(a->'sectores_ids') = 'array'
                         then (select array_agg(x) from jsonb_array_elements_text(a->'sectores_ids') x)
                         else null end
                );
...
-- ---------- pdt_visitas_en_alcance: devuelve sectores_ids en cada actividad ----------
                ...
                select jsonb_agg(jsonb_build_object(
                    ...
                    'actividades', coalesce((
                        select jsonb_agg(jsonb_build_object(
                            ...
                            'sectores_ids', case when a.sectores_ids is null then null
                                                  else to_jsonb(a.sectores_ids) end,
                            'materiales', coalesce((
                                select jsonb_agg(jsonb_build_object(
